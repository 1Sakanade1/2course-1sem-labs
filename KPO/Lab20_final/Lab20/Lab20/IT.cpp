...
namespace IT
{
	...
	char* IdTable::GetLexemaName()
	{
		...
		char numb[3]{};
		buffer[0] = 'L';
		_itoa_s(IdTable::noname_lexema_count, numb, 10);
		strcat_s(buffer, 5, numb);
		IdTable::noname_lexema_count++;
		return buffer;
...
#pragma region Literals
			...
			*(idStream) << std::setw(15) << "Идентификатор:" << std::setw(17) << "Тип данных:"<< std::setw(15) << "Значение:" << std::setw(27) << "Длина строки:" << std::setw(24) << "Первое вхождение:" << std::endl;

...
namespace LA
{
	void FindLex(const In::IN& source, LT::LexTable& lexTable, IT::IdTable& idTable)	//передаем параметры по ссылке
	{
		char* temp = new char[token_size] {};	
		int str_number = 1;						//задаем начальное положение номера строки и позиции
		int str_position = 1;
		In::IN in;

		for (int i = 0, j = 0; i < source.size; i++)	//проходим по исходному коду
		{
			if (in.code[(source.text[i])] != in.D && in.code[(source.text[i])] != in.O && (source.text[i]) != '|')//берем элемент из таблицы и сравниваем
			{
				temp[j++] = source.text[i];  //присваиваем значение из таблицы
				str_position++;				
				continue;
			}
			else

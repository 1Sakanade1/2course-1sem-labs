...
void Ping(const char* cHost, unsigned int Timeout, unsigned int RequestCount)
{
	...
	{
		int dwStatus = IcmpSendEcho(hIP,//дискриптор
			ipaddr, //ip адрес запроса
			SendData,//указатель на буфер эхо-запроса
			sizeof(SendData),//размер буфера эхо-запроса
			&option,//указатель на структуру с опциями запроса
			pIpe,//указатель на буфер эхо-ответа
			sizeof(ICMP_ECHO_REPLY) +
			sizeof(SendData),//размер буфера эхо-ответа
